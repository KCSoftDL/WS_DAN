...
def preprocess_for_train(image, height, width, bbox,
  ...
  with tf.compat.v1.name_scope(scope, 'distort_image', [image, height, width, bbox]):
    ...
    distorted_image = tf.image.resize(distorted_image, (int(height / ratio), int(width / ratio)))
    distorted_image = tf.squeeze(distorted_image, 0)
    distorted_image = tf.compat.v1.random_crop(distorted_image, (height, width, 3))
    distorted_bbox = bbox
    ...
    distorted_image = apply_with_random_selector(
        distorted_image,
        lambda x, method: tf.image.resize(x, [height, width], method=tf.image.ResizeMethod),
